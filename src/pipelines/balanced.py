...
class BalancedPipeline(VideoPipeline):
    def __init__(self, hf_token: str = None):
        self.vad = VADProcessor(method="pyannote")
        self.asr = ASRProcessor(model_size="medium", compute_type="int8")
    ...
    def process(self, video_path: str, output_dir: str) -> List[Dict[str, Any]]:
        ...
        audio_path = video_path # In real app, might extract audio first
        
        try:
            print("--- Balanced Pipeline ---")
            
            # 1. VAD
            vad_segments = self.vad.process(audio_path)
            if not vad_segments:
